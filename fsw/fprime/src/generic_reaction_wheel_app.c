...
static uart_info_t RW_UART[3] = {
    {.deviceString = &deviceName0[0],
     .handle = 2,
     .isOpen = PORT_CLOSED,
     .baud = 115200},
    {.deviceString = &deviceName1[0],
     .handle = 3,
     .isOpen = PORT_CLOSED,
     .baud = 115200},
    {.deviceString = &deviceName2[0],
     .handle = 4,
     .isOpen = PORT_CLOSED,
     .baud = 115200},
};
  ...
  while (CFE_ES_RunLoop(&GENERIC_RW_AppData.RunStatus) == true) {
    ...
    */
    ...
    status = CFE_SB_ReceiveBuffer(
        (CFE_SB_Buffer_t **)&GENERIC_RW_AppData.MsgPtr,
        GENERIC_RW_AppData.CommandPipe, CFE_SB_PEND_FOREVER);
...
int32 GENERIC_RW_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_RW App: Error Registering Events, RC = 0x%08lX\n",
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_RW App: Error Subscribing to HK request, RC = 0x%08lX\n",
        (unsigned long)status);
    return (status);
  }
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_RW App: Error Subscribing to Command, RC = 0x%08lX\n",
        (unsigned long)status);
  ...
  CFE_EVS_SendEvent(GENERIC_RW_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
                    ...
                    GENERIC_RW_APP_MAJOR_VERSION, GENERIC_RW_APP_MINOR_VERSION,
                    GENERIC_RW_APP_REVISION, GENERIC_RW_APP_MISSION_REV);
...
int32 GENERIC_RW_ReportHousekeeping(void) {
  ...
  */
  ...
  CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_RW_AppData.HkTlm.TlmHeader,
                     true);
...
int32 GENERIC_RW_ResetCounters(const GENERIC_RW_ResetCounters_t *Msg) {
  ...
  CFE_EVS_SendEvent(GENERIC_RW_COMMANDRST_INF_EID,
                    CFE_EVS_EventType_INFORMATION, "GENERIC_RW: RESET command");
...
int32 GENERIC_RW_Current_Momentum(const GENERIC_RW_Noop_t *Msg) {
  ...
  GENERIC_RW_AppData.HkTlm.Payload.CommandCounter++;
  CFE_EVS_SendEvent(GENERIC_RW_CMD_REQ_DATA_EID, CFE_EVS_EventType_DEBUG,
                    "Request Generic Reaction Wheel Data");
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (status > 0) {
      ...
      CFE_EVS_SendEvent(
          GENERIC_RW_CMD_REQ_DATA_EID, CFE_EVS_EventType_ERROR,
...
static int32_t GetCurrentMomentum(int wheel_number, double *momentum) {
  ...
  int32_t status = uart_write_port(&RW_UART[wheel_number], (uint8_t *)request,
                                   strlen(request));
  if (status < 0) {
...
int32 GENERIC_RW_Set_Torque(const GENERIC_RW_Cmd_t *Msg) {
  ...
  CFE_EVS_SendEvent(GENERIC_RW_CMD_SET_TORQUE_EID, CFE_EVS_EventType_DEBUG,
                    "Generic Reaction Wheel %d: Info, Set Torque Command "
                    "received (%d * 10^-4 N-m)",
                    cmd->wheel_number, cmd->data);
  torque = cmd->data;
  torque /= 10000.0; // units are 10^-4 Newton-meters (so we don't have to send
                     // floats in the command)

  sprintf(request, "SET_TORQUE=%10.4f", torque);
  status = uart_write_port(&RW_UART[cmd->wheel_number], (uint8_t *)request,
                           strlen(request));
...
bool GENERIC_RW_VerifyCmdLength(CFE_MSG_Message_t *Msg, uint16 ExpectedLength) {
  ...
  */
  if (ExpectedLength != ActualLength) {
    ...
    CFE_EVS_SendEvent(
        GENERIC_RW_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
